...
export default function PermissionsModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-40 flex justify-center items-center">
      <div className="bg-white max-w-6xl w-full rounded-lg shadow-lg p-6 relative overflow-y-auto max-h-[90vh]">
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="bg-gray-50 rounded shadow p-4 max-h-[500px] overflow-auto">
              {permissions.map((user) => (
                <li
                  key={user.user_id}
                  className={`py-2 px-4 border-b cursor-pointer ${
                    selectedUser === user.user_id ? 'bg-blue-100 font-semibold' : ''
                  }`}
