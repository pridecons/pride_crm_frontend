...
/** ========= Reused tiny helpers (kept intact) ========= */
const BTN_BASE =
...
const btnPrimaryStyle = {
  ...
  boxShadow: "0 1px 0 0 var(--theme-components-button-primary-shadow, transparent)",
};
const btnPrimaryFocus = { boxShadow: "0 0 0 2px var(--theme-components-button-primary-hoverBg, rgba(0,0,0,0.08))" };
...
const inputPillStyle = {
  ...
  border: "1px solid var(--theme-components-input-border, var(--theme-inputBorder))",
};

const tabBase = "inline-block p-4 border-b-2 rounded-t-lg cursor-pointer";
const tabActiveStyle = {
  color: "var(--theme-components-sidebar-activeText, var(--theme-primary))",
  borderBottomColor: "var(--theme-components-sidebar-activeText, var(--theme-primary))",
  borderBottomWidth: 2,
...
const tagSuccessStyle = {
  background: "var(--theme-components-tag-success-bg, rgba(34,197,94,0.10))",
  color: "var(--theme-components-tag-success-text, #16a34a)",
  border: "1px solid var(--theme-components-tag-success-border, rgba(34,197,94,0.35))",
};
...
const normalizeRoleKey = (r) => (r ?? "").toString().trim().toUpperCase().replace(/\s+/g, "_");
const pickBranchId = (u) => u?.branch_id ?? u?.user?.branch_id ?? u?.branch?.id ?? u?.assigned_user?.branch_id ?? null;
const pickEmployeeCode = (u) => u?.employee_code ?? u?.user?.employee_code ?? null;

const AssigneeAutoComplete = memo(function AssigneeAutoComplete({
  value, setValue,
  ...
  return (
    <div ref={wrapRef} className="relative w-[260px]">
      <label className="block text-[11px] font-bold text-[var(--theme-textSecondary)] mb-1.5">Assignee</label>
          ...
          {loading ? (
            ...
            options.map((opt) => (
              ...
              >
                ...
                {opt.role && <div className="text-[11px] text-[var(--theme-textSecondary)]">{opt.role}</div>}
              </button>
            ))
          ) : (
...
function DateInputPill({ label = "Date", value, onChange, placeholder = "yyyy-mm-dd" }) {
  return (
    <div className="relative w-[180px]">
      <label className="block text-[11px] font-bold text-[var(--theme-textSecondary)] mb-1.5">{label}</label>
      <div className="relative">
        <input
          type="date"
          value={value}
...
export default function FtLeadsPage() {
  ...
  // Team assignee filter (kept intact)
  ...
  const [appliedFilters, setAppliedFilters] = useState({ from: "", to: "", assignedUser: null });
  ...
  useEffect(() => {
    ...
    const roleGuess =
      info?.profile_role?.name ?? info?.role_name ?? info?.role ?? info?.user?.role_name ?? info?.user?.role ?? "";
    setMyRole(normalizeRoleKey(roleGuess));
    setMyBranchId(pickBranchId(info));
    setMyEmployeeCode(pickEmployeeCode(info));
  }, []);

  // search assignees (same as old)
  const searchAssignees = useCallback((q) => {
    ...
    assigneeTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        const { data } = await axiosInstance.get("/users-fiter/", { params: { search: query } });
  ...
  // Load responses and lock to FT
  const fetchResponses = useCallback(async () => {
    try {
      const { data } = await axiosInstance.get("/lead-config/responses/", { params: { skip: 0, limit: 100 } });
      setResponses(data || []);
      // find FT id dynamically (fallback to 4 if not found)
      const ftId = (data || []).find((r) => (r.name || "").toUpperCase() === "FT")?.id ?? 4;
      setResponseFilterId(ftId);
    } catch (error) {
      console.error("Error fetching responses:", error);
      // fallback to 4 if API fails
      setResponseFilterId(4);
    }
  }, []);
  ...
  // Fetch leads (same logic, but responseFilterId is always FT)
  const doFetchLeads = useCallback(async () => {
    ...
    try {
      ...
      if (viewType === "team" && assignedUser) params.assigned_user = assignedUser;
      ...
      ErrorHandling({ error, defaultError: "Failed to load FT leads" });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (responseFilterId) doFetchLeads();
  }, [doFetchLeads, responseFilterId]);

  const handleApplyFilters = () => {
    setAppliedFilters((prev) => ({ ...prev, from: (uiFromDate || "").trim(), to: (uiToDate || "").trim() }));
    setPage(1);
  };
  const handleResetFilters = () => {
    ...
    // keep FT locked; do not clear responseFilterId
    setPage(1);
  };
  ...
  const handleResponseChange = async (lead, newResponseId) => {
    ...
    try {
      await axiosInstance.patch(`/leads/${lead.id}/response`, { lead_response_id: id });
      setLeads((prev) => prev.map((l) => (l.id === lead.id ? { ...l, lead_response_id: id } : l)));
      toast.success("Response updated!");
      // if moved away from FT, it will naturally disappear on next fetch/re-render
      doFetchLeads();
    } catch (error) {
  ...
  const baseColumns = [
        ...
        return editId === lead.id ? (
          <input
            ...
            style={{
              ...
              border: "1px solid var(--theme-components-input-border, var(--theme-border))",
              outline: "none",
            }}
          />
        ) : hasName ? (
          <span className="truncate" title={lead.full_name} style={{ color: "var(--theme-text)" }}>
            {lead.full_name}
          </span>
        ) : (
          <span
    ...
    },
    {
      header: "Source",
      render: (lead) => {
        const name = lead?.lead_source?.name ?? (lead?.lead_source_id ? `ID: ${lead.lead_source_id}` : "N/A");
        return (
          <span className="inline-block px-2 py-1 text-xs font-medium rounded" style={tagSuccessStyle} title={name}>
            {name}
          </span>
        );
      },
    ...
    render: (lead) => {
      ...
      return (
        <span
          ...
          title={title || label}
          style={{
            color: isEmpty ? "var(--theme-textSecondary)" : "var(--theme-primary)",
          }}
        >
          {label}
        </span>
  ...
  const columns = useMemo(() => {
    ...
    if (viewType === "team") cols.splice(3, 0, assignedUserColumn);
    cols.push({
      header: "Actions",
      render: (lead) => (
        <div className="flex gap-2">
          <button
            ...
            title="Edit lead"
            className="group w-8 h-8 inline-flex items-center justify-center rounded-full transition-transform duration-150 hover:scale-110 active:scale-95"
    ...
    return cols;
  }, [viewType, responses, editId, ftFromDate, ftToDate, ftServiceType, router]);

  async function handleUpdateName(lead, nameRaw) {
    const name = (nameRaw || "").trim();
    if (!name) { setEditId(null); return; }
    if ((lead.full_name || "").trim()) { setEditId(null); return; }

    try {
      await axiosInstance.put(`/leads/${lead.id}`, { ...lead, full_name: name });
      setLeads((prev) => prev.map((l) => (l.id === lead.id ? { ...l, full_name: name } : l)));
    ...
    >
      ...
      <div className="shrink-0 w-full py-4" style={{ background: "var(--theme-background)" }}>
        ...
        <div className="w-full flex flex-wrap items-center justify-end gap-3 md:gap-4 p-2 md:p-2">
          ...
          <div className="flex mt-6 items-center gap-2">
            <button onClick={handleResetFilters} className={BTN_BASE} style={btnSoftStyle} title="Reset all filters">
              Reset
            </button>
            <button
      ...
      <FTModal
        ...
        onSave={async (payload) => {
          try {
            const ftId = responses.find((r) => r.name.toLowerCase() === "ft")?.id ?? responseFilterId ?? 4;
            await axiosInstance.patch(`/leads/${ftLead.id}/response`, { lead_response_id: ftId, ...payload });
      ...
      <CallBackModal
        ...
        onSave={async () => {
          if (!callBackDate)
            return ErrorHandling({ defaultError: "Call back date is required" });
          try {
