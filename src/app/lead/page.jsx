...
export default function LeadsTable() {
    ...
    try {
      ...
      const leadsWithComments = await Promise.all(
        items.map(async (item) => {
          const lead = { ...item.lead, assignment_id: item.assignment_id };

          try {
            const { data: commentsData } = await axiosInstance.get(`/leads/${lead.id}/comments`);
            lead.comment = commentsData.length > 0 ? commentsData[commentsData.length - 1].comment : null;
          } catch {
            lead.comment = null;
          }
  ...
  const handleSaveComment = async (lead) => {
    ...
    try {
      await axiosInstance.post(`/leads/${lead.id}/comments`, {
        user_id: userId,
        comment: lead.tempComment,
      });

      toast.success("Comment saved!");
      setLeads((prev) =>
        prev.map((l) => (l.id === lead.id ? { ...l, comment: lead.tempComment, tempComment: "" } : l))
      );
    } catch (error) {
  ...
  const filteredLeads = paginatedLeads.filter((lead) => {
    ...
    if (activeTab === "Old Leads") return lead.lead_response_id;
    return true;
  });
