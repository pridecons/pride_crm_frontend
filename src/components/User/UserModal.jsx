...
const roleKey = (r) => (r || "").toString().toUpperCase().replace(/\s+/g, ""); // "Super Admin" -> "SUPERADMIN"
...
export default function UserModal({
  ...
  const currentRoleKey = roleKey(currentUser?.role);
  const passwordRegex =
    /^(?=.*[0-9])(?=.*[!@#$%^&*()_\-+=\[\]{};':"\\|,.<>\/?]).{6,}$/;
              ...
              <div className="space-y-4">
                ...
                {formData.role !== "RESEARCHER" && (
                  <div>
                    ...
                    {/* IMPORTANT: default to dropdown while currentUser is unknown to avoid the "Loading branch…" trap */}
                    {!currentUser || currentRoleKey === "SUPERADMIN" ? (
                      <select
                        className="w-full p-3 border rounded-xl bg-white"
                        ...
                        return (
                          <>
                            <div className="w-full p-3 border rounded-xl bg-gray-50 text-gray-700">
                              {eff ? getBranchName(branches, eff) : "Loading branch…"}
                            </div>
                            <input type="hidden" name="branch_id" value={eff} />
                          </>
                        );
                      })()
                    )}
